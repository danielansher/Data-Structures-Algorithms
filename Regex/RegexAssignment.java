...
public class RegexAssignment{
    ...
    public static boolean testName(String name){
        return name.matches("[A-Z][a-z(\\-|\\'[A-Z])\\s]* [A-Z][a-z(\\-|\\'[A-Z])\\s]*");
    }
    ...
    public static boolean testAddress(String address){
        return address.matches("\\d{1,5}\\s[\\w \\.\\-]*\n[A-Z][\\w \\.\\-]*,?\\s[A-Z]{2}\\s[0-9]{5}(-\\d{4})?");
    }
    ...
    public static boolean testJavaExecutable(String java){
        return java.matches("(.|\n)*public\\sstatic\\svoid\\smain\\(String\\[\\]\\s[\\D][A-Za-z0-9_\\$]*\\)(.|\n)*");
    }
}